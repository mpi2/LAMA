...
def make_normalization_roi_qc_images(img_path, roi, out_dir):
    """
    ...
    sag_slice = np.flipud(arr[:, :, sag_slice_index])
    cor_slice = np.flipud(arr[:, cor_slice_index, :])
    ax_slice = np.flipud(arr[ax_slice_index, :, :])
    roi_props = []
    roi_props.append([sag_slice, roi_starts[0:2], roi_ends[0:2]])
    roi_props.append([cor_slice, [roi_starts[0], roi_starts[2]], [roi_ends[0], roi_ends[2]]])
    roi_props.append([ax_slice,  [roi_starts[1], roi_starts[2]], [roi_ends[1], roi_ends[2]]])
    # Draw roi on the slice
    widths = []
    heights = []
    images = []
    for slice_, roi_starts, roi_ends in roi_props:
