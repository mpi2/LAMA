...
-------
    ...
    $ lama_detect.py -c wt_registration_config_file_path -i dir_with_input_volumes -p project_directory_path
...
class LamaDetect(object):
    """
    Phenotype detection

    """
    def __init__(self, wt_config_path, mut_proj_dir, auto_select_baselines=None, wt_subset_file=None,
                 mut_subset_file=None, in_dir=None, n1=True):
        """
        Parameters
        ----------
        wt_config_path: str
            path to a wildtype cofig file. This file was generated when running the wild_type test set with lama.py
        mut_proj_dir: str
            path to root of project directory in which to save phenotype detection results
        wt_subset_file: str
            optional path to file containing list of wild types to use for comparison
        mut_subset_file: str
            optional path to file containing list of mutants to use for comparison
        in_dir: str
            path to directory containing mutant volumes to analsye. If None LamaDetevct will look for file in 'inputs'
            folder in the mut_proj_dir
        n1: bool
            whether to perform optional one against many analysis (reference range /z-score)
        auto_select_baselines: str/None
            if str should be of one of follwing:
                crl: uses crown-rump length matching
                volume: uses whole embryo volume comaprsion
                label_length: use length of a label skeleton. WE have used it successfully with a labeeld spine at E18.5
